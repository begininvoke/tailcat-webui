...
func (_q *UserQuery) QueryServers() *TailServerQuery {
...
func (_q *UserQuery) QueryClients() *TailClientQuery {
...
func (_q *UserQuery) QueryRoutes() *PublishedRouteQuery {
...
func (_q *UserQuery) QueryAuditEvents() *AuditEventQuery {
...
func (_q *UserQuery) WithServers(opts ...func(*TailServerQuery)) *UserQuery {
	query := (&TailServerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithClients(opts ...func(*TailClientQuery)) *UserQuery {
	query := (&TailClientClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithRoutes(opts ...func(*PublishedRouteQuery)) *UserQuery {
	query := (&PublishedRouteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithAuditEvents(opts ...func(*AuditEventQuery)) *UserQuery {
	query := (&AuditEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withAuditEvents; query != nil {
		if err := _q.loadAuditEvents(ctx, query, nodes,
			func(n *User) { n.Edges.AuditEvents = []*AuditEvent{} },
			func(n *User, e *AuditEvent) { n.Edges.AuditEvents = append(n.Edges.AuditEvents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadServers(ctx context.Context, query *TailServerQuery, nodes []*User, init func(*User), assign func(*User, *TailServer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tailserver.FieldUserID)
	}
...
func (_q *UserQuery) loadClients(ctx context.Context, query *TailClientQuery, nodes []*User, init func(*User), assign func(*User, *TailClient)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tailclient.FieldUserID)
	}
...
func (_q *UserQuery) loadRoutes(ctx context.Context, query *PublishedRouteQuery, nodes []*User, init func(*User), assign func(*User, *PublishedRoute)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(publishedroute.FieldUserID)
	}
...
func (_q *UserQuery) loadAuditEvents(ctx context.Context, query *AuditEventQuery, nodes []*User, init func(*User), assign func(*User, *AuditEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditevent.FieldUserID)
	}
	query.Where(predicate.AuditEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AuditEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
